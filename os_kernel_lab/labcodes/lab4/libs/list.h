...
//链表节点list_entry。没有包含传统的data数据域，而是在具体的数据结构中包含链表节点
struct list_entry {
 ...
 * is already in the list.
 * */
//将elm插入在listelm, listelm->next之间。（在表头插入）
static inline void
 ...
 * */
//在删除listelm，还将listelm节点置为空链状态（list_init(listelm)把listelm的prev、next指针指向自身）
//从而确保被删除的节点listelm不再指向链表中的其他节点
static inline void
