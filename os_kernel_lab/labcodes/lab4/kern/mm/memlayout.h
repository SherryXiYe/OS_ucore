...
// KSTACKSIZE：内核栈的空间大小。uCore在创建进程时分配了 2 个连续的物理页作为内核栈的空间。
...
struct Page {
    ...
    list_entry_t pra_page_link;     //可以用来建立描述各个页访问情况（比如根据访问先后）的链表 used for pra (page replace algorithm)
    uintptr_t pra_vaddr;            //可以用来记录此物理页对应的虚拟页起始地址 used for pra (page replace algorithm)
};
...
//pra_page_link可用来构造按页的第一次访问时间进行排序的一个链表，这个链表的开始表示第一次访问时间最远的页，链表结尾表示第一次访问时间最近的页。
