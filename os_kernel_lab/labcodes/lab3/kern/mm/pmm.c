...
alloc_pages(size_t n) {
    ...
    while (1)
    {
         ...
         swap_out(check_mm_struct, n, 0);   //一旦没有足够的物理页，则会使用swap_out函数将当前物理空间的某一页换出到外存
         //该函数会进一步调用sm（swap manager）中封装的swap_out_victim函数来选择需要换出的物理页，该函数是一个函数指针进行调用的，具体对应到了_fifo_swap_out_victim函数（因为在本练习中使用了FIFO替换算法)
    }
...
//调用alloc_page和page_insert函数来分配一个页面大小的内存，
//并使用线性地址la和PDT pgdir建立映射 pa<--->la
struct Page *
